/*
  copied from @tensorflow/tfjs-react-native, edited a little bit...
 */
/**
 * @license
...
import {fetch} from './tf_platform_react_native';
  ...
  async loadLocalAsset(weightsAssets: Asset[]): Promise<io.ModelArtifacts> {
    ...
    const weightsDataArray = await Promise.all(
      weightsAssets.map(async weightsAsset => {
        ...
        return util.encodeString(base64Weights, 'base64').buffer;
      }),
    );
